...
function x= method_grad (fun, xo, e, xlimites)
...
g = get_grad(fun, xo);     % Estima o grandiente da função no ponto xo
...
while (norm(g) >= e)            
    ...
    g = get_grad(fun, x);      % Calcula o grandiente da função no ponto x
    d = -g;                     % Determina a direção de busca    
